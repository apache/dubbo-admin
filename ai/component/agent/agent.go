...
func NewOrderOrchestrator(maxIterations int, stages ...*Stage) *OrderOrchestrator {
...
Outer:
	for i := 0; i < orchestrator.maxIterations; i++ {
		for _, order := range orchestrator.loop {
			...
			// Validate output is not nil to prevent nil pointer panic
			if output == nil {
