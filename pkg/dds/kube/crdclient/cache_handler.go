...
func (h *cacheHandler) onEvent(curr interface{}, event Event) error {
	...
	//if !ok {
	//	logger.Sugar().Warnf("New Object can not be converted to runtime Object %v, is type %T", curr, curr)
	//	return nil
	//}
	// currConfig := TranslateObject(currItem, h.schema.Resource().GroupVersionKind(), h.client.domainSuffix)

	for _, f := range h.handlers {
		// if you do this, If we do this, then we will build a cache by ourselves instead of the cache that comes with client-go
		//if event == model2.EventAdd {
		//	f.Resource.Add(h.schema, key, currConfig)
		//}
		//if event == model2.EventUpdate {
		//	f.Resource.Update(h.schema, key, currConfig)
		//}
		//if event == model2.EventDelete {
		//	f.Resource.Delete(h.schema, key)
		//}
		f.Resource.NotifyWithIndex(h.schema)
	}
	return nil
}
...
func createCacheHandler(cl *Client, schema collection.Schema, i informers.GenericInformer) *cacheHandler {
	...
	_, err := i.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			cl.queue.Push(func() error {
				return h.onEvent(newObj, EventUpdate)
			})
		},
		DeleteFunc: func(obj interface{}) {
			cl.queue.Push(func() error {
				return h.onEvent(obj, EventDelete)
			})
		},
	})
	if err != nil {
