...
type DynamicConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.DynamicConfig, err error)
...
// List lists all DynamicConfigs in the indexer.
func (s *dynamicConfigLister) List(selector labels.Selector) (ret []*v1beta1.DynamicConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.DynamicConfig))
	})
	return ret, err
}
...
type DynamicConfigNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.DynamicConfig, error)
...
// List lists all DynamicConfigs in the indexer for a given namespace.
func (s dynamicConfigNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.DynamicConfig, err error) {
...
// Get retrieves the DynamicConfig from the indexer for a given namespace and name.
func (s dynamicConfigNamespaceLister) Get(name string) (*v1beta1.DynamicConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("dynamicconfig"), name)
	}
	return obj.(*v1beta1.DynamicConfig), nil
}
