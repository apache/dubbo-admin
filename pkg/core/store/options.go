...
func (l *GetOptions) Predicate(r coremodel.Resource) bool {
	if l.Mesh != "" && r.ResourceMesh() != l.Mesh {
		return false
	}

	if l.Version != "" && r.ResourceMeta().ResourceVersion != l.Version {
		return false
	}

	if len(l.Labels) > 0 {
		for k, v := range l.Labels {
			if r.ResourceMeta().Labels[k] != v {
				return false
			}
		}
	}
...
func (l *ListOptions) Predicate(r coremodel.Resource) bool {
	...
	if l.NameEquals != "" && r.ResourceMeta().Name != l.NameEquals {
		return false
	}

	if l.NameContains != "" && !strings.Contains(r.ResourceMeta().Name, l.NameContains) {
		return false
	}

	if l.ApplicationContains != "" && !strings.Contains(r.ResourceMeta().Labels[meshproto.ApplicationLabel], l.ApplicationContains) {
		return false
	}
