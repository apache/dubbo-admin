    ...
    <search-table :search-domain="searchDomain">
      ...
      <template #bodyCell="{ text, column, record }">
        <template v-if="column.dataIndex === 'ruleName'">
          <RouterLink class="rule-name-link" :to="`${basePath}/edit/${record.ruleName}`">
            {{ text }}
          </RouterLink>
        </template>
        ...
        <template v-else-if="column.dataIndex === 'operation'">
          ...
          <a-popconfirm
            :title="t('routerRuleDomain.deleteConfirm', { ruleName: record.ruleName })"
            :ok-button-props="{ loading: removingRuleName === record.ruleName }"
            @confirm="remove(record.ruleName)"
          >
            <a-button type="link" danger :loading="removingRuleName === record.ruleName">
              {{ t('delete') }}
            </a-button>
          </a-popconfirm>
        </template>
...
async function remove(ruleName: string) {
  if (removingRuleName.value) return
  removingRuleName.value = ruleName
  try {
    const res = await deleteRouterRuleAPI(props.kind, ruleName)
    if (res.code === HTTP_STATUS.SUCCESS) {
      message.success(t('routerRuleDomain.deleteSuccess', { rule: ruleLabel.value }))
      await searchDomain.onSearch()
    }
  } catch (error: any) {
    message.error(error?.message || t('routerRuleDomain.deleteFailed'))
  } finally {
    removingRuleName.value = ''
  }
}
