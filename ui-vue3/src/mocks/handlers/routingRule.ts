...
export const routingRuleHandlers: HttpHandler[] = [
  http.get(`${base}/condition-rule/search`, () => {
    ...
    const list: RoutingRule[] = Array.from({ length: total }, () => ({
      ruleName: 'app_' + randomString(2, 10),
      ruleGranularity: Math.random() > 0.5,
      enable: Math.random() > 0.5,
      createTime: '2024-01-01 00:00:00'
    }))
    return success<PaginatedData<RoutingRule>>({
      pageInfo: { Total: total, NextOffset: '0' },
      list
    })
  }),

  http.get(`${base}/condition-rule/:ruleName`, ({ params }) => {
    const detail: RoutingRuleDetail = {
